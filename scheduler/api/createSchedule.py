...
# the size (in minutes) of the invidually alloted chunks of time
TIME_CHUNK = 15


def createSchedule(user):
    myTasks = api.models.Task.objects.filter(user=user).order_by(
        'dueDate')  # grabs only the users tasks ans filters them by due date
    lastTaskTime = findLastTask(user)
    arraySize = int(math.ceil(
        (lastTaskTime-datetime.datetime.now())/datetime.timedelta(minutes=TIME_CHUNK)))
    ...
    for task in regularTaskList:
        if task.taskLength < TIME_CHUNK and task.taskLength > 0:
            ...
            tempTask = api.models.Task(id=api.models.generate_unique_task_id(), taskName=task.taskName, dueDate=(
            ...
            tempTask = api.models.Task(id=api.models.generate_unique_task_id(
    ...
    for task in subdividedTaskList:
        print("Name: " + str(task.taskName) + "   DueDate: " +
              str(task.dueDate) + "   taskLength: " + str(task.taskLength))
